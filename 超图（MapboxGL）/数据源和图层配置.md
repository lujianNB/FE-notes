...
**源的类型由"type"属性指定，并且必须是vector（矢量切片），raster（栅格切片，比vector多了一个属性tileSize控制线图层的宽度），raster-dem（栅格高程，比 raster 多了一个属性 encoding），geojson，image，video之一。**
...
```
// 此处的name为数据源的名称，需要有唯一性。
map.addSource('name',option) // 设置数据源
map.getSource('name') // 获取数据源
map.removeSource('name') // 删除数据源
```

# 图层（Layers）

**图层指定了如何渲染数据源提供的数据，id和type必填。**

* id有唯一性也是图层名

* type可选：circle（圆点），symbol（符号），line（线），heatmap（热力图），fill（填充），fill-extrusion（三维填充），background（背景），raster（栅格），hillshade（坡面阴影）

* 案例（symbol与circle的区别是，symbol可以加图片及文字标注）
...
"layers": [
    {
        ...
        "layout": { // 布局类属性
            ...
            "text-field": "", // 文本所对应的字段（可选，默认值为 ""），glyphs（字体符号）必填
...
```
// 此处的id为图层的名称，需要有唯一性。
map.addLayer(option) // 设置图层
map.getLayer('id') // 获取图层
map.removeLayer('id') // 删除图层
```

# 注意事项

* 设置图层时应先设置数据源再设置图层，删除时应先删除图层再删除数据源，其中symbol设置自定义图标还需要先配置图片源，删除时也是要先删除图层才能删除图片源。

```
// 图片源设置（使用时symbol图层里面的icon-image与name对应即可）
map.addImage('name',imageUrl) // 添加图片源
map.hasImage('name') // 是否存在图片源
map.removeImage('name') // 删除图片源
...
map.loadImage(
    ...
    (error, image) => {
        ...
        resizeImageBitmap(image, 22, 22).then((resizedBitmap) => {
            map.addImage('name', resizedBitmap)
        })
    }
)
// resizeImageBitmap可以设置图片的大小，开发中很多图层的时候设置图例如果不去控制原始图片的大小，不能很好的配置自己想要的大小
...
* 样式如果需要多样性可以用mapbox表达式去写，这里不多描述，后面单独讲。

* 写点击悬停的图层样式时最好使用覆盖图层的方式，因为线图层，面图层里的元素的层级没办法去设置改变，如果两个元素有重叠，当悬停的是下面的那个元素，效果会被覆盖在上面的元素遮盖。